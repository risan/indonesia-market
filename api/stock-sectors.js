...
module.exports = async (req, res) => {
  try {
    ...
    const data = response.data.map((sector, idx) => ({
      id: sector.Id,
      name: sector.Name,
      stocks_url: `https://indonesia-market.vercel.app/api/stocks?sector_id=${sector.Id}`,
      subsectors: subSectors[idx].map(subsector => ({
        id: subsector.Id,
        name: subsector.Name,
        stocks_url: `https://indonesia-market.vercel.app/api/stocks?subsector_id=${subsector.Id}`,
      })),
    }));
